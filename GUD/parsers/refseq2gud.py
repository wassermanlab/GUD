...
help_msg = """%s
inserts features from both the UCSC's "ncbiRefSeq" and
"ncbiRefSeqSelect" tables into GUD.
...
def refgene_to_gud(genome, version, dummy_dir="/tmp/", remove=False, test=False, threads=1):
    ...
    for table in ["ncbiRefSeq", "ncbiRefSeqSelect"]:

        # Download data
        data_file, url = _download_data(genome, table, dummy_dir)
...
def _download_data(genome, table, dummy_dir="/tmp/"):
    ...
    ftp_file = os.path.join(url, "%s.txt.gz" % table)
    data_file = os.path.join(dummy_dir, "%s.txt.gz" % table)
